...
def build_payload(params: dict, stream: bool):
    payload = {
        "model": MODEL,
        "messages": [
    ...
    # You can remove if you prefer fully stochastic runs.
    payload["seed"] = 42

    return payload

def stream_once(params: dict):
    """
    Returns:
      text (str), ttft_s (float|None), total_s (float), usage (dict|None)
    """
    payload = build_payload(params, stream=True)
...
def main():
    ...
    csv_path = f"results/phase1_sampling_grid_{ts}.csv"
    ...
    with open(csv_path, "w", newline="", encoding="utf-8") as f:
        ...
        for params in GRID:
            ...
            text, ttft_s, total_s, usage = stream_once(params)
            ...
            if not usage:
                ...
                usage, fallback_total_s = nonstream_usage_only(params)
